...
export const PROGRAM_AREA_SCALE = 0.39
...
function room(
  ...
  return {
    ...
    areaSqm: overrides.areaSqm ?? Math.round(template.defaultAreaSqm * PROGRAM_AREA_SCALE),
...
function upperVerticalCores(floors: number[]): PlacedRoom[] {
  return floors.map((floor) => room('verticalCore', floor, 50, 20, 8, 8, { locked: true }))
}
...
export function createTertiaryHospitalPlan(): HospitalPlan {
  ...
  return {
    ...
    rooms: [
      ...
      room('verticalCore', -1, 50, 20, 8, 8, { locked: true }),
      ...
      room('verticalCore', 0, 50, 20, 8, 8, { locked: true }),
