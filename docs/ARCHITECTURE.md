...
El `Top` es la vista inicial del producto porque orienta el trabajo hacia comparacion de alternativas antes que edicion de detalle. Su UI debe ser compacta: ranking, KPIs y acciones primarias visibles; formula, penalizaciones y datos completos en modales de detalle. La vista `Analisis` concentra los cuellos de botella: presion por estancia, casos bloqueados y estado visible, con lectura operativa bajo demanda. El panel de simulacion muestra solo parametros y cuatro KPIs principales; metricas extendidas y reglas abiertas pasan a modal. Los paneles laterales son contextuales: aparecen en `Planificador` y `Simulacion`, donde sirven para plantas, construccion, casos clinicos o parametros, y pueden ocultarse desde la barra superior; `Analisis` y `Servicios` trabajan sin asides para no introducir controles irrelevantes.
