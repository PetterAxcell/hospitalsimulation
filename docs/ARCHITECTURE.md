...
La UI compacta ya no vive solo en `App.tsx`: el ranking esta en `frontend/src/features/top/`, saturacion en `frontend/src/features/saturation/`, el selector operativo de simulacion en `frontend/src/features/simulation/` y las primitivas compartidas en `frontend/src/components/ui/`. El motor de preview tambien empieza a separarse por dominio: `simulation.ts` orquesta recorridos, KPIs y posicionamiento; `clinicalCases.ts` contiene la libreria YAML de casos, parser y ponderacion; `staffSimulation.ts` contiene roles de personal, rutas de turno y resumen de staff; `circulation.ts` mantiene red de puertas/pasillos y `architectureRules.ts` evalua reglas espaciales.

El siguiente paso arquitectonico recomendado es dividir `SimulationCanvas.tsx` entre escena Phaser, controles de replay y capas visuales. Esa frontera prepara la futura entrada de agentes reales de paciente, medico, enfermeria, celadores y tecnicos sin hacer crecer de nuevo un unico archivo central.
