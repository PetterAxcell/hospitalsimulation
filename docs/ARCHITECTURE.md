...
El `Top` es la vista inicial del producto porque orienta el trabajo hacia comparacion de alternativas antes que edicion de detalle. Su UI debe ser compacta: ranking, KPIs y acciones primarias visibles; formula, penalizaciones y datos completos en modales de detalle. La vista `Analisis` concentra los cuellos de botella: presion por estancia, casos bloqueados y estado visible, con lectura operativa bajo demanda. Cada pestana expone una accion contextual en la barra superior que abre su propio modal: registrar propuesta en `Top`, herramientas en `Planificador`, escenario en `Simulacion`, lectura en `Analisis` y resumen en `Servicios`. El panel de simulacion muestra solo parametros y cuatro KPIs principales; metricas extendidas y reglas abiertas pasan a modal. Los paneles laterales son contextuales: aparecen en `Planificador` y `Simulacion`, donde sirven para plantas, construccion, casos clinicos o parametros, y pueden ocultarse desde la barra superior. En pantallas moviles los laterales se ocultan por defecto y la accion contextual pasa a ser el acceso principal a herramientas, para mantener el canvas, el replay o la tabla como superficie dominante.
