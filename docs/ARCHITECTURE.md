...
El `Top` es la vista inicial del producto porque orienta el trabajo hacia comparación de alternativas antes que edición de detalle. Su UI debe ser compacta: ranking, KPIs y acciones primarias visibles; fórmula, penalizaciones y datos completos en modales de detalle. La vista `Análisis` concentra los cuellos de botella: presión por estancia, casos bloqueados y estado visible, con lectura operativa bajo demanda. `Top`, `Análisis` y `Servicios` mantienen una acción contextual en la barra superior porque no necesitan paneles laterales persistentes. `Planificador` y `Simulación` conservan laterales solo como resumen ligero: plantas, construcción, casos clínicos, personal, parámetros y KPIs aparecen como bloques pequeños, y cada bloque abre su propio modal de detalle cuando hace falta operar o revisar información completa. En escritorio, esos laterales pueden ocultarse desde la barra superior; en pantallas móviles se ocultan por defecto y aparece un único acceso contextual de herramientas para mantener el canvas, el replay o la tabla como superficie dominante.

El planificador mantiene el arrastre de bloques dentro del canvas hasta que el usuario suelta el puntero. Mientras se arrastra se pinta una previsualizacion local; al finalizar se actualiza el `HospitalPlan`. Esta separacion evita que los calculos globales de reglas, simulacion y ranking se ejecuten decenas de veces por segundo. La camara del planificador encaja el plano por ancho y permite pan por arrastre cuando el usuario pulsa una zona vacia, de forma que moverse por la planta no compite con mover salas, puertas o tiradores de redimensionado.
...
La UI compacta ya no vive solo en `App.tsx`: el shell corporativo esta separado en `frontend/src/components/AppHeader.tsx` y `frontend/src/components/WorkspaceTabs.tsx`; el inspector de estancias esta en `frontend/src/features/planning/RoomInspector.tsx`; el ranking esta en `frontend/src/features/top/`, el analisis de cuellos de botella en `frontend/src/features/saturation/`, el selector operativo de casos y el panel compacto de simulacion estan en `frontend/src/features/simulation/`, la barra de replay vive en `frontend/src/components/SimulationControlsBar.tsx` y las primitivas compartidas en `frontend/src/components/ui/`. El motor de preview tambien empieza a separarse por dominio: `simulation.ts` orquesta recorridos, KPIs y posicionamiento; `clinicalCases.ts` contiene la libreria YAML de casos, parser y ponderacion; `staffSimulation.ts` contiene roles de personal, rutas de turno y resumen de staff; `circulation.ts` mantiene red de puertas/pasillos y `architectureRules.ts` evalua reglas espaciales.

El siguiente paso arquitectonico recomendado es dividir `SimulationCanvas.tsx` entre escena Phaser pura y capas visuales de habitaciones, ocupacion, agentes y cuidados. Esa frontera prepara la futura entrada de agentes reales de paciente, medico, enfermeria, celadores y tecnicos sin hacer crecer de nuevo un unico archivo central.
